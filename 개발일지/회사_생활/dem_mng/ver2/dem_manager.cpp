...
void dem_manager::delete_map() const
{
	if (!preload_dem_cache.empty())
		preload_dem_cache.clear();
}

void dem_manager::clean_up_expired_time() const
{
	using std::chrono::system_clock;
	using std::chrono::hours;
	static std::mutex mutex;
	static std::thread task;
	static std::chrono::system_clock::time_point clear_cache_at;

	{
		std::lock_guard<std::mutex> lock(mutex);
		clear_cache_at = system_clock::now() + hours(5);
	}

	if (task.joinable()) return;

	task = std::thread ( [this]()->void
		{
			do
			{
				std::this_thread::sleep_until(clear_cache_at);
			} 
			while (system_clock::now() < clear_cache_at);
