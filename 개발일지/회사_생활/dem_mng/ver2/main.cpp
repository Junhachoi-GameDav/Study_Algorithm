...
void handle_client(boost::asio::ip::tcp::socket&& socket, dem_manager& dem_mng)
{
	try
	{
		...
		if (std::isnan(temp_x) or std::isinf(temp_x) or temp_x < -219825.99 or temp_x > 913114.23)
			throw std::exception("x is invalid");

		boost::asio::read(socket, boost::asio::buffer(&temp_y, sizeof(temp_y)));
		if (std::isnan(temp_y) or std::isinf(temp_y) or temp_y < -442558.89 or temp_y > 877525.22)
			throw std::exception("y is invalid");
